...
const AddEdit = () => {
    ...
    useEffect(() => {
        if (id) {
            loader(true)
            ApiClient.get('api/particpant/type/detail', { id }).then(res => {
                if (res.success) {
                    let value = res.data
                    let payload = form
                ...
                <div className="pprofile1">
                    ...
                    <div className='flex items-center mb-8'>
                        <Tooltip placement="top" title="Back">
                            ...
                            <Link to="/participant-type" className="!px-4  py-2 flex items-center justify-center  rounded-lg shadow-btn hover:bg-[#F3F2F5] border transition-all  mr-3"><i className='fa fa-angle-left text-lg'></i></Link>
                        </Tooltip>
                        <div>
                            <h3 className="text-2xl font-semibold text-[#111827]">
                                {form && form.id ? 'Edit' : 'Add'}  Participant Type
                            </h3>
                            <p class="text-sm font-normal text-[#75757A]">Here you can see all about your Participant Type</p>
                        </div>
                    </div>
