...
const Content = (p) => {
    ...
    const getData = (p = {}) => {
        ...
        ApiClient.get('api/contentManagement/list', filter).then(res => {
            if (res.success) {
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            ApiClient.delete('api/contentManagement', { id: id }).then(res => {
                if (res.success) {
    ...
    const statusChange = (itm) => {
        ...
        if (window.confirm(`Do you want to ${status == 'active' ? 'Activate' : 'Deactivate'} this`)) {
            loader(true)
            ApiClient.put(`api/contentManagement`, { id: itm.id, status }).then(res => {
                if (res.success) {
                    getData()
                }
                loader(false)
